...
// Get all personas
router.get('/api/council/personas', async (req: Request, res: Response) => {
  try {
...
router.get('/api/council/personas/:role', async (req: Request, res: Response) => {
  try {
...
// Update persona
router.put('/api/council/personas/:role', async (req: Request, res: Response) => {
  try {
    const { role } = req.params;
    const personaPath = path.join(__dirname, 'personas', `${role}.json`);
    ...
    await fs.writeFile(personaPath, JSON.stringify(req.body, null, 2));
...
// Get round templates
router.get('/api/council/templates', async (req: Request, res: Response) => {
  const templates = {
    standard: {
      name: 'Standard',
...
router.post('/api/council/templates', async (req: Request, res: Response) => {
  try {
